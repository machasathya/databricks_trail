...
table_name = "FACT_FPR_YEARLY_KPI_TARGETS"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "fpr_yearly_kpi_targets")).select("*")

# COMMAND ----------

# writing data to processed layer before generating surrogate key
df.write.parquet(processed_location+'FPR_YEARLY_KPI_TARGETS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,"FACT_FPR_YEARLY_KPI_TARGETS","FPR_YEARLY_KPI_TARGETS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_LOGISTICS_FPR_YEARLY_KPI_TARGETS")

# COMMAND ----------

# inserting log record to last_execution_details data to SQL
