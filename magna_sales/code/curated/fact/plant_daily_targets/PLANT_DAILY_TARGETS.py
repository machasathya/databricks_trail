...
table_name = "FACT_PLANT_DAILY_TARGETS"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "PLANT_DAILY_TARGETS")).select("*")
...
df.write.parquet(processed_location+'PLANT_DAILY_TARGETS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,"FACT_PLANT_DAILY_TARGETS","PRODUCTION_PLAN",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_LOGISTICS_PLANT_DAILY_TARGETS")
