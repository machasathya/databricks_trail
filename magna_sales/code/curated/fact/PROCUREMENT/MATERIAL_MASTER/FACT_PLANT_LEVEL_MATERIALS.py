...
from pyspark.sql.functions import col, lit, expr, when, unix_timestamp, current_date,lpad,max as _max,sum
...
table_name = "FACT_PLANT_LEVEL_MATERIALS"
...
# reading data from curated layer and creating active,inactive flag value
marc = spark.sql("select MATNR as MATERIAL_NUMBER,WERKS as PLANT_ID,LVORM as DELETION_FLAG from {}.{}".format(curated_db_name, "MARC"))

temp1 = marc.withColumn("ACTIVE",when((col("DELETION_FLAG").isNull()),lit(1)).otherwise(lit(0))).withColumn("INACTIVE",when((col("DELETION_FLAG").isNull()) == False,lit(1)).otherwise(lit(0)))

# COMMAND ----------

# aggregating data on plant id value
final_df = temp1.groupBy("PLANT_ID").agg(sum("ACTIVE").alias("ACTIVE"),sum("INACTIVE").alias("INACTIVE"))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'PLANT_LEVEL_MATERIALS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PLANT_LEVEL_MATERIALS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_PLANT_LEVEL_MATERIALS")
