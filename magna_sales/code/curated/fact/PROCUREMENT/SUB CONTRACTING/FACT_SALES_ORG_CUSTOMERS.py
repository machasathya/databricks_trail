...
table_name = "FACT_SALES_ORG_CUSTOMERS"
...
knvv = spark.sql("select distinct kunnr,vkorg from {}.{}".format(curated_db_name, "KNVV"))

final_df = knvv.select(col("kunnr").alias("CUST_ID"),col("VKORG").alias("SALES_ORG"))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'SALES_ORG_CUSTOMERS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"SALES_ORG_CUSTOMERS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_SALES_ORG_CUSTOMERS")

# COMMAND ----------

# insert log record to last execution details table
