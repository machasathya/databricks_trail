...
from pyspark.sql.functions import col, lit, expr, when, unix_timestamp, current_date,lpad,max as _max,split
...
table_name = "FACT_RM_NEW_VENDOR_DEVELOPMENT"
...
df = spark.sql("select * from {}.{}".format(curated_db_name,"RM_NEW_VENDOR_DEVELOPMENT"))

# COMMAND ----------

# adding leading 0's to vendor id and material id fields
final_df = df.withColumn("MATERIAL_ID",lpad(col("MATERIAL_ID"),18,'0')).withColumn("VENDOR_ID",lpad(col("VENDOR_ID"),10,'0'))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'RM_NEW_VENDOR_DEVELOPMENT', mode='overwrite')
...
#   transpose_bucket.createOrReplaceTempView("zfi_cag")
  ...
  query = """select
{fact_name}.MATERIAL_DESCRIPTION,
{fact_name}.VENDOR_NAME,
{fact_name}.ADDRESS,
SAMPLE_QTY_MT,
R_AND_D_RESULT_DATE,
R_AND_D_RESULT,
R_AND_D_REASON_FOR_REJECTION,
TRIAL_ORDER_QTY_MT,
TRIAL_ORDER_RECEIPT_DATE,
TRIAL_ORDER_RATE_INR_MT,
TRIAL_DATE,
TRIAL_RESULT_DATE,
TRIAL_ORDER_RESULT,
TRIAL_ORDER_REASON_FOR_REJECTION,
BULK_TRIAL_QTY_MT,
BULK_TRIAL_RATE_INR_MT,
BULK_ORDER_RECEIPT_DATE,
BULK_TRIAL_DATE,
BULK_TRIAL_RESULT_DATE,
BULK_TRIAL_RESULT,
BULK_ORDER_REASON_FOR_REJECTION,
...
d = surrogate_mapping_hil(csv_data,table_name,"RM_NEW_VENDOR_DEVELOPMENT",processed_db_name)
