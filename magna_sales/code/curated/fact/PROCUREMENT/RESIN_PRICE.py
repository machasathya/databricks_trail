...
df = spark.sql("select * from {}.{}".format(curated_db_name,"RESIN_PRICE")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# extracting columns other than plants
cols = [c for c in df.columns if c not in {'DATE', 'PRICE_TYPE'}]

# COMMAND ----------

# pivot plant cols to rows
df1 = df.selectExpr('DATE', 'PRICE_TYPE', "stack({}, {})".format(len(cols), ', '.join(("'{}', {}".format(i, i) for i in cols)))).withColumnRenamed("col0","PLANT").withColumnRenamed("col1","PRICE_VALUE")

# COMMAND ----------

# defining plant id column
final_df = df1.withColumn("PLANT_ID",split(df1['plant'], '_').getItem(0)).drop("plant")

# COMMAND ----------

# writing data to processed location before generating surrogate keys
final_df.write.parquet(processed_location+'RESIN_PRICE', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"RESIN_PRICE",processed_db_name)
