...
table_name = "FACT_PLANT_LEVEL_VENDOR"
...
LFM1 = spark.sql("select * from {}.{}".format(curated_db_name, "LFM1"))

# COMMAND ----------

# selecting columns
data = LFM1.selectExpr("LIFNR","EKORG as PLANT_ID","ERDAT","SPERM")

# COMMAND ----------

# defining active and inactive columns
temp1 = data.withColumn("IN_ACTIVE",when(trim(col("SPERM")) == 'X',lit(1)).otherwise(lit(0))).withColumn("ACTIVE",when(trim(col("SPERM")).isNull(),lit(1)).otherwise(lit(0)))

# COMMAND ----------

# selecting columns
temp2 = temp1.selectExpr("PLANT_ID","IN_ACTIVE","ACTIVE")

# COMMAND ----------

# aggregating data on plant id
final_df = temp2.groupBy("PLANT_ID").agg(sum("IN_ACTIVE").alias("IN_ACTIVE"),sum("ACTIVE").alias("ACTIVE"))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'PLANT_LEVEL_VENDOR', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PLANT_LEVEL_VENDOR",processed_db_name)
