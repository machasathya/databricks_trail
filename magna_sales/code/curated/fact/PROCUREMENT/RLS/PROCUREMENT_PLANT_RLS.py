...
table_name = "FACT_PROCUREMENT_RLS"
...
# table_name = "FACT_PROCUREMENT_PLANT_RLS"
# last_processed_time = datetime.now()
# last_processed_time_str = last_processed_time.strftime("%Y-%m-%d %H:%M:%S")

# COMMAND ----------

# reading groups data from curated layer
grps_df=spark.sql("select * from {}.{}".format(curated_db_name, "GROUPS")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# reading procurement roles data from curated layer
bi_grps_df=spark.sql("select * from {}.{}".format(curated_db_name, "BI_GROUPS_PROCUREMENT")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# mapping mail id to plant id
...
rls_final.write.mode('overwrite').parquet(processed_location+"FACT_PROCUREMENT_RLS")
