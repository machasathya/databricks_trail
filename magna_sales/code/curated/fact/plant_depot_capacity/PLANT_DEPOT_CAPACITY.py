...
# fetching data from ADF Parameters
...
table_name = "FACT_PLANT_DEPOT_CAPACITY"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "PLANT_DEPOT_CAPACITY")).select("*")

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
df.write.parquet(processed_location+'PLANT_DEPOT_CAPACITY', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,'FACT_PLANT_DEPOT_CAPACITY',"PLANT_DEPOT_CAPACITY",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_LOGISTICS_PLANT_DEPOT_CAPACITY")
