...
table_name = "FACT_NSR_PACKING_COST"
...
df=spark.sql("select PRODUCT_LINE,FINANCIAL_YEAR,APRIL,MAY,JUNE,JULY,AUGUST,SEPTEMBER,OCTOBER,NOVEMBER,DECEMBER,JANUARY,FEBRUARY,MARCH from {}.{}".format(curated_db_name, "NSR_SBU1_PACKING_COST"))

# COMMAND ----------

# summing up month columns and filtering columns that doesn't have data
...
num1_df = sum1_df.withColumn("MONTH",when(col('MARCH').isNotNull(),lit(14)).when(col('FEBRUARY').isNotNull(),lit(13)).when(col('JANUARY').isNotNull(),lit(12)).when(col('DECEMBER').isNotNull(),lit(11)).when(col('NOVEMBER').isNotNull(),lit(10)).when(col('OCTOBER').isNotNull(),lit(9)).when(col('SEPTEMBER').isNotNull(),lit(8)).when(col('AUGUST').isNotNull(),lit(7)).when(col('JULY').isNotNull(),lit(6)).when(col('JUNE').isNotNull(),lit(5)).when(col('MAY').isNotNull(),lit(4)).when(col('APRIL').isNotNull(),lit(3)))
...
std_col = [c for c in int_df1.columns if c not in {"PRODUCT_LINE","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot data based on month field
pivot_df = int_df1.selectExpr("PRODUCT_LINE","FINANCIAL_YEAR", "stack({}, {})".format(len(std_col), ', '.join(("'{}', {}".format(i, i) for i in std_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","NSR_PACKING_COST")

# COMMAND ----------

# changing FY and month and adding starting date of calendar month
...
final_df.write.parquet(processed_location+'NSR_PACKING_COST', mode='overwrite')
...
final_df = surrogate_mapping_hil(csv_data,table_name,"NSR_PACKING_COST",processed_db_name)
