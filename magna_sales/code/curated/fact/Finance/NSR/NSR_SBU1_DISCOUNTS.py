...
df=spark.sql("select STATE,PRODUCT_LINE,FINANCIAL_YEAR,APRIL,MAY,JUNE,JULY,AUGUST,SEPTEMBER,OCTOBER,NOVEMBER,DECEMBER,JANUARY,FEBRUARY,MARCH from {}.{}".format(curated_db_name, "NSR_SBU1_DISCOUNTS"))

# COMMAND ----------

# summing up month columns and removing columns which doesn't have values
...
std_col = [c for c in int_df1.columns if c not in {"STATE","PRODUCT_LINE","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data on month field
pivot_df = int_df1.selectExpr("STATE","PRODUCT_LINE","FINANCIAL_YEAR", "stack({}, {})".format(len(std_col), ', '.join(("'{}', {}".format(i, i) for i in std_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","NSR_DISCOUNTS")

# COMMAND ----------

# changing FY and month to starting date of calendar month
...
final_df.write.parquet(processed_location+'NSR_DISCOUNTS', mode='overwrite')
...
final_df = surrogate_mapping_hil(csv_data,table_name,"NSR_DISCOUNTS",processed_db_name)
...
# insert log record into laste execution details table
