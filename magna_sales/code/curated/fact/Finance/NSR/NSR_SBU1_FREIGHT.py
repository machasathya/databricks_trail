...
table_name = "FACT_FREIGHT_BUDGET_INR_MT"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "FREIGHT_BUDGET_SBU1")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# summing columns and filtering out the columns that doesn't have values
sum_df = df.agg(sf.sum('APRIL').alias('APRIL'),sf.sum('MAY').alias('MAY'),sf.sum('JUNE').alias('JUNE'),sf.sum('JULY').alias('JULY'),sf.sum('AUGUST').alias('AUGUST'),sf.sum('SEPTEMBER').alias('SEPTEMBER'),sf.sum('OCTOBER').alias('OCTOBER'),sf.sum('NOVEMBER').alias('NOVEMBER'),sf.sum('DECEMBER').alias('DECEMBER'),sf.sum('JANUARY').alias('JANUARY'),sf.sum('FEBRUARY').alias('FEBRUARY'),sf.sum('MARCH').alias('MARCH'))

# COMMAND ----------

month_df = sum_df.withColumn("MONTH",when(col('MARCH').isNotNull(),lit(15)).when(col('FEBRUARY').isNotNull(),lit(14)).when(col('JANUARY').isNotNull(),lit(13)).when(col('DECEMBER').isNotNull(),lit(12)).when(col('NOVEMBER').isNotNull(),lit(11)).when(col('OCTOBER').isNotNull(),lit(10)).when(col('SEPTEMBER').isNotNull(),lit(9)).when(col('AUGUST').isNotNull(),lit(8)).when(col('JULY').isNotNull(),lit(7)).when(col('JUNE').isNotNull(),lit(6)).when(col('MAY').isNotNull(),lit(5)).when(col('APRIL').isNotNull(),lit(4)))
x_list = month_df.select('MONTH').collect()[0][0]

# COMMAND ----------

df_final =  df.select(df.columns[:x_list])
pivot_col = [c for c in df_final.columns if c not in {"PRODUCT_LINE","FREIGHT_CLASSIFICATION","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data on month field
df1 = df_final.selectExpr("PRODUCT_LINE","FREIGHT_CLASSIFICATION","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","FREIGHT_BUDGET_INR")

# COMMAND ----------

# changing FY and month to starting date of the calendar month
...
final_df.write.parquet(processed_location+'FREIGHT_BUSGET_SBU1', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"FREIGHT_BUDGET_SBU1",processed_db_name)
