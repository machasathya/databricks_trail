...
table_name = "FACT_WP_NSR_PERCENTAGE"
...
# Reading data from WC_ROCE Table and dropping Last Updated details from the table
df=spark.sql("select * from {}.{}".format(curated_db_name, "NSR_SBU3_WP")).drop("LAST_UPDATED_DT_TS")
...
std_col = [c for c in int_df1.columns if c not in {"PRODUCT_LINE","SEGMENT","ZONE","FINANCIAL_YEAR"}]

# COMMAND ----------

# Pivot Transformations and adding Date all the records
pivot_df = int_df1.selectExpr("PRODUCT_LINE","SEGMENT","ZONE","FINANCIAL_YEAR", "stack({}, {})".format(len(std_col), ', '.join(("'{}', {}".format(i, i) for i in std_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","PERCENTAGE")
...
# Saving the contents of final dataFrame as a Parquet file, and preserving the schema
final_df.write.parquet(processed_location+'NSR_SBU3_WP', mode='overwrite')
