...
table_name = "FACT_WP_FREIGHT_BUDGET"
last_processed_time = datetime.now()
last_processed_time_str = last_processed_time.strftime("%Y-%m-%d %H:%M:%S")

# COMMAND ----------

# variables to run the notebok manually
# meta_table = "fact_surrogate_meta"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "WP_FREIGHT_BUDGETS")).drop("LAST_UPDATED_DT_TS")
...
std_col = [c for c in int_df1.columns if c not in {"FREIGHT_CLASSIFICATION","PRODUCT_LINE","SEGMENT","ZONE","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month
pivot_df = int_df1.selectExpr("FREIGHT_CLASSIFICATION","PRODUCT_LINE","SEGMENT","ZONE","FINANCIAL_YEAR", "stack({}, {})".format(len(std_col), ', '.join(("'{}', {}".format(i, i) for i in std_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","BUDGET_PERCENTAGE")
...
final_df.write.parquet(processed_location+'WP_FREIGHT_BUDGET', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"WP_FREIGHT_BUDGET",processed_db_name)
