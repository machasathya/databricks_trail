...
table_name = "FACT_ASPIRE_GL_BUDGETS_TRANS"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "ASPIRE_GL_BUDGETS")).drop(*["LAST_UPDATED_DT_TS"])

# COMMAND ----------

# summing up month columns
...
# seperating cols other than month
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","GL_GROUP","UOM","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot month columns to rows
pivot_df = trans_df.selectExpr("PLANT_ID","GL_GROUP","UOM","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","TARGET")

# COMMAND ----------

# changing FY and month to starting date of each month
...
# generating date range dataframe
...
# duplicating data for all days in a month
...
df5 = df4.withColumn("GL_GROUP",regexp_replace("GL_GROUP"," ","_"))

# COMMAND ----------

# pivot on plant and date
final_df = df5.groupBy("PLANT_ID","DATE").pivot("GL_GROUP").sum("TARGET")

# COMMAND ----------

# writing data to processed location before generating surrogate keus
final_df.write.parquet(processed_location+'ASPIRE_GL_BUDGETS_TRANS', mode='overwrite')
...
#   transpose_bucket.createOrReplaceTempView("zfi_cag")
  ...
  query = """select
 CONTRACT_LABOUR,
GENERAL_REPAIRS_AND_MAINTENANCE,
LABOUR,
PACKING,
STORES_AND_SPARES,
WAGES_AND_BENEFITS
...
d = surrogate_mapping_hil(csv_data,table_name,"ASPIRE_GL_BUDGETS_TRANS",processed_db_name)
