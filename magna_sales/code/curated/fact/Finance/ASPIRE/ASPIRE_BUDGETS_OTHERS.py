...
table_name = "FACT_ASPIRE_BUDGETS_OTHERS"
...
# Modifying the Dates to Financial Dates are mapped to the the previous year except for January, February, March
...
# filtering data for which data is not present
final_df = final_df.where("date is not null")

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'ASPIRE_BUDGETS_OTHERS', mode='overwrite')
