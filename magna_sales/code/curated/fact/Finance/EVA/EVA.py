...
eva_data = spark.sql("select * from {}.{} where particulars != '% Growth or Degrowth'".format("cur_prod", "EVA")).drop("LAST_UPDATED_DT_TS")
...
final_df.write.parquet(processed_location+'EVA', mode='overwrite')
...
d1 = surrogate_mapping_hil(csv_data,table_name,"EVA",processed_db_name)

# COMMAND ----------

# writing data to processed location after generating surrogate keys
d1.write.mode('overwrite').parquet(processed_location+table_name)

# COMMAND ----------

# inserting log record LAST_EXECUTION_DETAILS table
