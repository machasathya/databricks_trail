...
table_name = "FACT_PUTTY_MIS_OTHER_COST_BUDGET"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "PUTTY_MIS_OTHER_COST_BUDGET")).drop(*["LAST_UPDATED_DT_TS","GL_GROUP","GL_DESCRIPTION"]).withColumn("GL_CODE",lpad(col("GL_CODE"),10,'0'))
...
# selecting columns other than month
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","GL_CODE","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month
pivot_df = trans_df.selectExpr("PLANT_ID","GL_CODE","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","BUDGET_VALUE")
...
final_df.write.parquet(processed_location+'PUTTY_MIS_OTHER_COST_BUDGET', mode='overwrite')
