...
table_name = "FACT_PUTTY_MIS_TARGET_MASTER"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "PUTTY_MIS_TARGET_MASTER")).drop(*["LAST_UPDATED_DT_TS","MATERIAL_DESCRIPTION"])
...
df4 = df3.withColumn("DATE",df3['DATE_STR'].cast(DateType())).drop('FINANCIAL_YEAR').drop('LAST_UPDATED_DT_TS').drop('MONTH').drop('MONTH_NUMBER').drop('YEAR').drop('DATE_STR')

# COMMAND ----------

# adding leading 0's to material id field
final_df = df4.withColumn("MATERIAL_ID",lpad(col("MATERIAL_ID"),18,'0'))

# COMMAND ----------

# Writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'PUTTY_MIS_TARGET_MASTER', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PUTTY_MIS_TARGET_MASTER",processed_db_name)
