...
table_name = "FACT_PUTTY_MIS_CONVERSION_COST_ACTUALS"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "PUTTY_MIS_CONVERSION_COST_ACTUALS")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# summing up month columns and removing months that doesn't have values
...
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","PARTICULARS","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month
pivot_df = trans_df.selectExpr("PLANT_ID","PARTICULARS","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","CONVERSION_COST_VALUE")
...
final_df.write.parquet(processed_location+'PUTTY_MIS_CONVERSION_COST_ACTUALS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PUTTY_MIS_CONVERSION_COST_ACTUALS",processed_db_name)
