...
#  reading surrogate metadata fron SQL
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "BOM_TARGETS")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

#copying data to APR 1st of every year and adding leading 0's to material id field
final_df = df.withColumn("DATE",sf.concat((df["FINANCIAL_YEAR"].cast(IntegerType()))-1,lit("-04-01"))).withColumn("MATERIAL_ID",lpad(col("MATERIAL_ID"),18,'0')).drop("FINANCIAL_YEAR")

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'BOM_TARGETS', mode='overwrite')
...
#   transpose_bucket.createOrReplaceTempView("zfi_cag")
  ...
  query = """select
  {fact_name}.PER_PRODUCTION_MT_TARGET,
...
d = surrogate_mapping_hil(csv_data,table_name,"BOM_TARGETS",processed_db_name)
...
# insert log record into LAST_EXECUTION_DETAILS table
