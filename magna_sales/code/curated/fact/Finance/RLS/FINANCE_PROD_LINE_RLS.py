...
table_name = "FACT_FINANCE_PROD_LINE_RLS"
...
# reading data from curated layer
bi_grps_df=spark.sql("select * from {}.{}".format(curated_db_name, "BI_GROUPS_FINANCE_PRODUCT_LINE")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# defining mailid to product line mapping
rls=bi_grps_df.alias("df1").join(grps_df.alias("df2"),on=[col("df1.GROUP")==col("df2.GROUP")],how='left')

rls_df = rls.select("USER_MAIL_ID","PRODUCT_LINE")

# COMMAND ----------

# writing data to processed layer
rls_df.write.mode('overwrite').parquet(processed_location+"FACT_FINANCE_PROD_LINE_RLS")
