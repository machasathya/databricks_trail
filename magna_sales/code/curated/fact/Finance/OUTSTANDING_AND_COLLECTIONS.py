...
table_name = "FACT_OUTSTANDING_AND_COLLECTIONS"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "OUTSTANDING_AND_COLLECTIONS"))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
df.write.parquet(processed_location+'OUTSTANDING_AND_COLLECTIONS', mode='overwrite')
...
#   transpose_bucket.createOrReplaceTempView("zfi_cag")
  ...
  query = """select
   SBU,
   PRODUCT_LINE_FOR_OUTSTANDING,
   SALE_TYPE,
   OVERDUE_PERIOD_START_IN_DAYS
...
d = surrogate_mapping_hil(csv_data,table_name,"OUTSTANDING_AND_COLLECTIONS",processed_db_name)
