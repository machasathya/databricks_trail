...
table_name = "FACT_PF_MIS_FIXED_COST_BUDGETS"
...
df=spark.sql("select PLANT_ID,GL_DESCRIPTION,GL_ACCOUNT,FINANCIAL_YEAR,APRIL,MAY,JUNE,JULY,AUGUST,SEPTEMBER,OCTOBER,NOVEMBER,DECEMBER,JANUARY,FEBRUARY,MARCH from {}.{}".format(curated_db_name, "PF_MIS_FIXED_COST_BUDGETS")).drop("LAST_UPDATED_DT_TS").withColumn("GL_ACCOUNT",lpad(col("GL_ACCOUNT"),10,'0'))

# COMMAND ----------

# summing up columns and filtering the months that doesn't have value
...
# speerating columns other than months
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","GL_DESCRIPTION","GL_ACCOUNT","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on date
pivot_df = trans_df.selectExpr("PLANT_ID","GL_DESCRIPTION","GL_ACCOUNT","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","TARGET")
...
final_df.write.parquet(processed_location+'PF_MIS_FIXED_COST_BUDGETS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PF_MIS_FIXED_COST_BUDGETS",processed_db_name)
