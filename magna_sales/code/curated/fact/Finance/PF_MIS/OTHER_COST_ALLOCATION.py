...
table_name = "FACT_OTHER_COST_ALLOCATION"
...
df = spark.sql("""select * from {}.{} """.format(curated_db_name, "OTHER_COST_ALLOCATION")).drop(col("LAST_UPDATED_DT_TS"))

# COMMAND ----------

# summing up of month columns and filtering months that doesn't have values
...
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","SEGMENT","PRODUCT_TYPE","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month
pivot_df = trans_df.selectExpr("PLANT_ID","SEGMENT","PRODUCT_TYPE","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","COST_VALUE")
...
final_df.write.parquet(processed_location+'OTHER_COST_ALLOCATION', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"OTHER_COST_ALLOCATION",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surroagte key
d.write.mode('overwrite').parquet(processed_location+"FACT_OTHER_COST_ALLOCATION")
