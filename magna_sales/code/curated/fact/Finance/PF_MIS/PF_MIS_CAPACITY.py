...
table_name = "FACT_PF_MIS_CAPACITY"
...
df = spark.sql("""select * from {}.{} """.format(curated_db_name, "PF_MIS_CAPACITY")).drop(col("LAST_UPDATED_DT_TS"))

# COMMAND ----------

# summing up month fields and filtering the months which doesn't have values
...
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","PF_CATEGORY","UOM","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month
pivot_df = trans_df.selectExpr("PLANT_ID","PF_CATEGORY","UOM","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","CAPACITY")

# COMMAND ----------

# changing FY and month to calendar year and adding starting date of the month field
...
final_df.write.parquet(processed_location+'PF_MIS_CAPACITY', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PF_MIS_CAPACITY",processed_db_name)
