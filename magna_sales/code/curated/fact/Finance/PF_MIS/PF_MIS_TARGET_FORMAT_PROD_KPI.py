...
table_name = "FACT_PF_MIS_TARGET_FORMAT_PROD_KPI"
...
df=spark.sql("select PARTICULARS,UOM,KPI,STAGE,PLANT_ID,FINANCIAL_YEAR,APRIL,MAY,JUNE,JULY,AUGUST,SEPTEMBER,OCTOBER,NOVEMBER,DECEMBER,JANUARY,FEBRUARY,MARCH from {}.{}".format(curated_db_name, "PF_MIS_KPI_TARGETS")).drop("LAST_UPDATED_DT_TS")
...
pivot_col = [c for c in trans_df.columns if c not in {"PARTICULARS","UOM","KPI","STAGE","PLANT_ID","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data on month
pivot_df = trans_df.selectExpr("PARTICULARS","UOM","KPI","STAGE","PLANT_ID","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","TARGET")

# COMMAND ----------

# converting FY and month to clanedar month and adding start date of the month 
...
final_df.write.parquet(processed_location+'PF_MIS_TARGET_FORMAT_PROD_KPI', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"PF_MIS_TARGET_FORMAT_PROD_KPI",processed_db_name)
