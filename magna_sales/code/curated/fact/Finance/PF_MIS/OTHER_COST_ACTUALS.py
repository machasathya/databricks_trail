...
table_name = "FACT_OTHER_COST_ACTUALS"
...
df = spark.sql("""select * from {}.{} """.format(curated_db_name, "OTHER_COST_ACTUALS")).drop(col("LAST_UPDATED_DT_TS"))

# COMMAND ----------

# summing up month columns and filtering the columns that doesn't have values
...
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","PARTICULARS","PF_CATEGORY","SEGMENT","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month
pivot_df = trans_df.selectExpr("PLANT_ID","PARTICULARS","PF_CATEGORY","SEGMENT","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","ACTUAL_VALUE")

# COMMAND ----------

# changing FY and month to claendar year and adding start day of month value
...
final_df.write.parquet(processed_location+'OTHER_COST_ACTUALS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"OTHER_COST_ACTUALS",processed_db_name)
