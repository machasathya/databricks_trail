...
table_name = "FACT_CONVERSION_COST_TARGETS_PRODUCT_TYPE"
...
# Reading data from CONVERSION_COST_TARGETS_PRD_TYP Table and dropping last updated column
df = spark.sql("""select * from {}.{} """.format(curated_db_name, "CONVERSION_COST_TARGETS_PRD_TYP")).drop(col("LAST_UPDATED_DT_TS"))
...
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","PRODUCT_TYPE","PARTICULARS","MEASURE","UOM","FINANCIAL_YEAR"}]

# COMMAND ----------

# Pivot Transformations and adding Date to all the records
pivot_df = trans_df.selectExpr("PLANT_ID","PRODUCT_TYPE","PARTICULARS","MEASURE","UOM","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","TARGET")

# COMMAND ----------

# Modifying the Dates to Financial Dates where January, February, March are mapped to the the next year
...
final_df.write.parquet(processed_location+'CONVERSION_COST_TARGETS_PRD_TYP', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"CONVERSION_COST_TARGETS_PRD_TYP",processed_db_name)
