...
table_name = "FACT_OTHER_COST_TARGETS"
...
df = spark.sql("""select PLANT_ID,PARTICULARS,SEGMENT,FINANCIAL_YEAR,APRIL,MAY,JUNE,JULY,AUGUST,SEPTEMBER,OCTOBER,NOVEMBER,DECEMBER,JANUARY,FEBRUARY,MARCH from {}.{} """.format(curated_db_name, "OTHER_COST_TARGETS")).drop(col("LAST_UPDATED_DT_TS"))

# COMMAND ----------

# summing up of month fields and removing months that doesn't have values
...
# seperatig columns other than months
pivot_col = [c for c in trans_df.columns if c not in {"PLANT_ID","PARTICULARS","SEGMENT","FINANCIAL_YEAR"}]

# COMMAND ----------

# pivot the data based on month field
pivot_df = trans_df.selectExpr("PLANT_ID","PARTICULARS","SEGMENT","FINANCIAL_YEAR", "stack({}, {})".format(len(pivot_col), ', '.join(("'{}', {}".format(i, i) for i in pivot_col)))).withColumnRenamed("col0","MONTH").withColumnRenamed("col1","TARGET_VALUE")

# COMMAND ----------

# changing FY and month and adding start date of the month value
...
# writing data to processed layer before generating surroagte key
final_df.write.parquet(processed_location+'OTHER_COST_TARGETS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,table_name,"OTHER_COST_TARGETS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surroagte key
