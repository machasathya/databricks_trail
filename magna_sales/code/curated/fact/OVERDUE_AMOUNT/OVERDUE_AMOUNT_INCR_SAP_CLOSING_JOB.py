...
# checking sap_closing_run log whether the run for sap closing month is completed or not
...
# Defining last date of the month
...
# div_knvv_df = div_knvv_df.alias("dc").join(division, on = [div_knvv_df["division_id"] == division["DIVISION_ID"]],
#                                    how='left').select([col("dc." + cols) for cols in div_knvv_df.columns]+ [col("DIVISION_NAME")])
div_knvv_df.createOrReplaceTempView("DIV_KNVV")

# COMMAND ----------

#balance_amount_n = balance_amount.alias("oc").join(div_knvv_df, on = [balance_amount["KUNNR"] == div_knvv_df["KUNNR"]],
                                   #how='left').select([col("oc." + cols) for cols in balance_amount.columns]+ [col("division_id")])
# balance_amount = balance_amount.dropDuplicates()
...
# display(transpose_bucket_final_df)
