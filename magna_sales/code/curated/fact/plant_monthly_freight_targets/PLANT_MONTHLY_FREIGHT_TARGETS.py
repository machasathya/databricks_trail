...
# initialising object to processmetadatautility class
...
table_name = "FACT_PLANT_MONTHLY_FREIGHT_TARGETS"
...
df=spark.sql("select * from {}.{}".format(curated_db_name, "PLANT_MONTHLY_FREIGHT_TARGETS")).select("*")

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
df.write.parquet(processed_location+"PLANT_MONTHLY_FREIGHT_TARGETS", mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,"FACT_PLANT_MONTHLY_FREIGHT_TARGETS","PLANT_MONTHLY_FREIGHT_TARGETS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_LOGISTICS_PLANT_MONTHLY_FREIGHT_TARGETS")
