...
# reading data from curated layer and adding leading 0's to material id column
df=spark.sql("select * from {}.{}".format(curated_db_name, "STC_TARGETS")).select("*")
df=df.withColumn("MATERIAL_ID",lpad(col("MATERIAL_ID"),18,'0'))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
df.write.parquet(processed_location+'STC_TARGETS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,"FACT_STC_TARGETS","STC_TARGETS",processed_db_name)

# COMMAND ----------

# writing data to processed layer location after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_LOGISTICS_STC_TARGETS")

# COMMAND ----------

# inerting log record to last execution details table
