...
table_name = "FACT_QUALITY_PROCESS_AUDIT"
...
df_SBU1=spark.sql("select * from {}.{}".format(curated_db_name, "PROCESS_AUDIT_SBU1"))
df_SBU2=spark.sql("select * from {}.{}".format(curated_db_name, "PROCESS_AUDIT_SBU2"))
df_SBU3=spark.sql("select * from {}.{}".format(curated_db_name, "PROCESS_AUDIT_SBU3"))

# COMMAND ----------

# merging SBU's data frame to one
final_df = df_SBU1.union(df_SBU2).union(df_SBU3)
final_df = final_df.withColumn("NO_OF_REPETITIVE_OBSERVATIONS_AFTER_CLOSURE",when((col("NO_OF_REPETITIVE_OBSERVATIONS_AFTER_CLOSURE").isNull()),0).otherwise(col("NO_OF_REPETITIVE_OBSERVATIONS_AFTER_CLOSURE")))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'PROCESS_AUDIT', mode='overwrite')
...
FUNCTION,
NO_OF_OBSERVATIONS,
NO_OF_OBSERVATIONS_CLOSED,
NO_OF_CRITICAL_OBSERVATIONS,
NO_OF_CRITICAL_OBSERVATIONS_CLOSED,
NO_OF_REPETITIVE_OBSERVATIONS_AFTER_CLOSURE,
REMARKS,
...
d = surrogate_mapping_hil(csv_data,"FACT_QUALITY_PROCESS_AUDIT","PROCESS_AUDIT",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_QUALITY_PROCESS_AUDIT")
