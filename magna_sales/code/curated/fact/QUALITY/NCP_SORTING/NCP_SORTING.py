...
# table_name = "FACT_QUALITY_NCP_SORTING"
...
SBU1_df = spark.sql("select * from {}.{}".format(curated_db_name,"NCP_SORTING_SBU1")).drop('LAST_UPDATED_DT_TS')
SBU2_df = spark.sql("select * from {}.{}".format(curated_db_name,"NCP_SORTING_SBU2")).drop('LAST_UPDATED_DT_TS')
SBU3_df = spark.sql("select * from {}.{}".format(curated_db_name,"NCP_SORTING_SBU3")).drop('LAST_UPDATED_DT_TS')

# COMMAND ----------

# merging all SBU's data and drop null records
final_df = SBU1_df.union(SBU2_df).union(SBU3_df)
df = final_df.dropna(how='all')

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
df.write.parquet(processed_location+'NCP_SORTING', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data, table_name, "NCP_SORTING", processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_QUALITY_NCP_SORTING")
