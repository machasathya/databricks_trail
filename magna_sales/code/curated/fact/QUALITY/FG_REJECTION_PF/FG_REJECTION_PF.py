...
table_name = "FACT_QUALITY_FG_REJECTION_PF"
...
# fetching surrogate metadata table
...
final_df = spark.sql("select * from {}.{}".format(curated_db_name, "FG_REJECTION_PF"))

# COMMAND ----------

# adding leading 0's to material id field
final_df=final_df.withColumn("MATERIAL_ID",lpad(col("MATERIAL_ID"),18,'0'))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'FG_REJECTION_PF', mode='overwrite')
...
#   transpose_bucket.createOrReplaceTempView("zfi_cag")
  ...
  query = """select
  {fact_name}.WEIGHT_PER_PC,
  {fact_name}.QUANTITY_EA,
  {fact_name}.QUANTITY_KG,
  {fact_name}.REJECTION_JUSTIFICATION
...
d = surrogate_mapping_hil(csv_data,table_name,"FG_REJECTION",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_QUALITY_FG_REJECTION_PF")
