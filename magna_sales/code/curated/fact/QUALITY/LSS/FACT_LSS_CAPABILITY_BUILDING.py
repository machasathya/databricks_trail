...
table_name = "FACT_QUALITY_LSS_CAPABILITY_BUILDING"
...
final_df = spark.sql("""select * from {}.{} """.format(curated_db_name, "LSS_CAPABILITY_BUILDING"))

# COMMAND ----------

# writing data to processed layer before generating surrogate keys
final_df.write.parquet(processed_location+'LSS_CAPABILITY_BUILDING', mode='overwrite')
...
#   transpose_bucket.createOrReplaceTempView("zfi_cag")
  ...
  CLEARED_ASSESSMENT_PARTICIPANTS,
...
d = surrogate_mapping_hil(csv_data,table_name,"LSS_CAPABILITY_BUILDING",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_QUALITY_LSS_CAPABILITY_BUILDING")

# COMMAND ----------

# inserting log recod to last execution details table
