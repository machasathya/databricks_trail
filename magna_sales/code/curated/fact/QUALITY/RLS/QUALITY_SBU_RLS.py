...
table_name = "FACT_QUALITY_SBU_RLS"
...
# reading data from curated layer
bi_grps_df=spark.sql("select * from {}.{}".format(curated_db_name, "BI_GROUPS_QUALITY_SBU")).drop("LAST_UPDATED_DT_TS")

# COMMAND ----------

# defining mail id to RLA mapping
rls=bi_grps_df.alias("df1").join(grps_df.alias("df2"),on=[col("df1.GROUP")==col("df2.GROUP")],how='left')

rls_df = rls.select("USER_MAIL_ID","SBU","PRODUCT_TYPE").withColumn("RLS",concat(col("SBU"),lit("_"),col("PRODUCT_TYPE")))

# COMMAND ----------

# writing data to processed layer location
rls_df.write.mode('overwrite').parquet(processed_location+"FACT_QUALITY_SBU_RLS")
