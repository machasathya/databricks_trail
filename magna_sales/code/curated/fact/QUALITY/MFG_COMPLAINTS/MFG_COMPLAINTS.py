...
table_name = "FACT_QUALITY_MFG_COMPLAINTS"
...
df_sbu1 = spark.sql("""select * from {}.{} """.format(curated_db_name, "MFG_COMPLAINTS_SBU1"))
df_sbu2 = spark.sql("""select * from {}.{} """.format(curated_db_name, "MFG_COMPLAINTS_SBU2"))
df_sbu3 = spark.sql("""select * from {}.{} """.format(curated_db_name, "MFG_COMPLAINTS_SBU3"))
...
final_df.write.parquet(processed_location+'MFG_COMPLAINTS', mode='overwrite')
...
BATCH_NUMBER,
COMPLAINT_STATUS,
ACTION_IDENTIFIED_FROM_INVESTIGATION,
ACTION_STATUS,
...
d = surrogate_mapping_hil(csv_data,table_name,"MFG_COMPLAINTS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_QUALITY_MFG_COMPLAINTS")
