...
table_name = "FACT_PLANT_MONTHLY_TARGETS"
last_processed_time = datetime.now()
last_processed_time_str = last_processed_time.strftime("%Y-%m-%d %H:%M:%S")

# COMMAND ----------

# fetching surroagte metadata from SQL
...
# reading data from curated layer and adding leading 0's to material id field
df=spark.sql("select * from {}.{}".format(curated_db_name, "plant_monthly_targets")).select("*")
df=df.withColumn("MATERIAL_ID",lpad(col("MATERIAL_ID"),18,'0'))

# COMMAND ----------

# writing date to processed layer before generating surrogate keys
df.write.parquet(processed_location+'PLANT_MONTHLY_TARGETS', mode='overwrite')
...
d = surrogate_mapping_hil(csv_data,"FACT_PLANT_MONTHLY_TARGETS","PLANT_MONTHLY_TARGETS",processed_db_name)

# COMMAND ----------

# writing data to processed layer after generating surrogate keys
d.write.mode('overwrite').parquet(processed_location+"FACT_LOGISTICS_PLANT_MONTHLY_TARGETS")
