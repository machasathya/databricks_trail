...
from pyspark.sql.functions import col, lit, expr, to_date, to_timestamp, sum, min, when, count, concat,rank,upper
...
# increasing broadcasttimeout to 36000 seconds
...
csv_data = pmu.get_data(cursor=db_cursor, col_lookup="LAST_EXECUTION_DETAILS", value="FACT_SALES_INVOICE", column="FACT_NAME",schema_name = schema_name)
pmu.close(db_connector, db_cursor)

# COMMAND ----------

# defining last executed time value
...
# PARQUET NULL ISSUE
vbrk = spark.sql("select vbeln , fkdat, kdgrp,kunag, kunrg, vkorg,vtweg,inco1,inco2, kurrf, konda, waerk, fkart,zzveh_no, zzlr_no,zzdr_no, zzewaybill_no, regio, bzirk, knumv,fksto FROM {}.{} where rfbsk == 'C' AND fkart NOT IN ('S1','S2','S3')".format(curated_db_name, "VBRK"))

# COMMAND ----------

# reading incremental data from curated layer 
vbrp = spark.sql("select vbeln,aubel,aupos,lgort, matnr, matkl, posnr, voleh, spart, vrkme, werks, cmpre,arktx, prsdt, vgbel, vgpos, kondm, fkimg, brgew, netwr, mwsbp,volum,bwtar FROM {}.{} where to_timestamp(LAST_UPDATED_DT_TS, 'yyyy-MM-dd HH:mm:ss.SSS') >= '".format(curated_db_name, "VBRP") + last_executed_time_str + "' and fkimg > 0")
...
# joining invoice header and item tables
...
# joining vbrk and knvv and fetching knvv columns
...
# joining vbrp and excdtl and fetching excdtl columns
...
# joining vbkd and vbrp and fetching vbkd columns
...
# joining vbrp and vbap and fetching vbap columns
...
# MAGIC %sql
# MAGIC
# MAGIC drop table if exists c_gt_konv

# COMMAND ----------

# joining vbrk and wa_taxturn and fetching wa_taxturn columns
...
c_gt_konv.write.saveAsTable("c_gt_konv")

c_gt_konv = spark.read.table("c_gt_konv")

# COMMAND ----------

#selecting vbrp columns
...
# fetching conversion values from marm to convert the quantity ea to volume
ls_final_1 = ls_final_base.alias("base").join(marm.alias("marm"), on=['matnr'],
                                           how='left').select(
...
# fetching the plant, report plant id and is_plant value
...
# creating temp plant id as if it it plant then plant or else bwtar
...
#fetching conv value from zsbu3
...
# joining with bkpf on inv no and fetching doc no company code and item
...
#  joining with bseg and fetching bupla
...
# joining with c_gt_excise and fetching exdat,exnum
...
# joining with knvv and fetching sales district, customer group, purchase group, sales ofc info
...
# joining kunrg with vbpa and fetching adress number and payer id
...
# joining with vbpa where parvw is AG and fetching sold cust id, address and agent address
...
# joining with vbpa and fetching agent part cust id and address id
...
# joining with vbpa and fetching bill to cust id and address id
...
# joining with vbpa to fetch ship to cust id and address
...
# joining vbpa to fetch sales_person number
...
# joining with vbap anf fetching route details
...
# joining with vbpa and fetching vendor details
...
# joining with vbak and fetching  po related information
...
# selecting all columns and adding new column called cust_type based on kunrg and kunag
...
# joining with kna1 and fetching pincode
...
# joining with export pincodes data frame and adding it
...
# joining with konv and fetching conditional types and values
...
# function to generate assessable value
...
# function to define secondaru frieght
...
# calculating various cond values upon cond num and item combination
...
# extracting assessible value and secondary freight
...
# adding assessible value2 and secondarry freight to main datframe
...
# partition based on sold to party cust id and billing date to define new customer
...
# partition based on product line and date to define new customers at product line level
...
# partition based on product line and kunag to find the first customer  upon customer and product line level
...
# clubbing all first cust flag values to main dataframe
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS temp_first;

# COMMAND ----------

# reading data from processed layer
dim_cust=spark.sql("select cust_id,customer_key from {}.{}".format(processed_db_name,"dim_customer"))
dim_prod=spark.sql("select product_line,product_line_key from {}.{}".format(processed_db_name,"dim_product_line"))
fact_inv=spark.sql("select sold_to_party_key,product_line_key  from {}.{} group by sold_to_party_key,product_line_key".format(processed_db_name,"fact_sales_invoice"))

# COMMAND ----------

# fetching cust id 
first_1=fact_inv.alias("inv").join(dim_cust.alias("cust"),on=[fact_inv["sold_to_party_key"]== dim_cust["customer_key"]],how='inner').select(col("cust.cust_id"),col("inv.product_line_key"))

# COMMAND ----------

# fetching product line
first_2=first_1.alias("fst").join(dim_prod.alias("prd"),on=[first_1["product_line_key"]== dim_prod["product_line_key"]],how='inner').select(col("fst.cust_id"),col("prd.product_line"))

# COMMAND ----------

first_2.write.saveAsTable("temp_first")

# COMMAND ----------

first_cust_matr = spark.sql("select * from temp_first")
cust_dist = spark.sql("select distinct(cust_id) from temp_first")
prod_dist = spark.sql("select distinct(product_line) from temp_first")

# COMMAND ----------

# adding FIRST_CUST_FLAG_1
cust_flag=ls_final_24.alias("final").join(cust_dist.alias("frst_cust"),on =  [ls_final_24["kunag"] ==  cust_dist["cust_id"]],how='left').withColumn("FIRST_CUST_FLAG_1",when(col("final.kunag") == col("frst_cust.cust_id"),0).otherwise(col('final.FIRST_CUST_FLAG'))).select(
     [col("final." + cols) for cols in ls_final_24.columns]+[col("FIRST_CUST_FLAG_1")])

# COMMAND ----------

# adding FIRST_PRD_SALES_FLAG_1
prd_sales_flg=cust_flag.alias("cust_flg").join(prod_dist.alias("frst_prod"),on =  [cust_flag["product_line"] ==  prod_dist["product_line"]],how='left').withColumn("FIRST_PRD_SALES_FLAG_1",when(col("cust_flg.product_line") == col("frst_prod.product_line"),0).otherwise(col('cust_flg.FIRST_PRD_SALES_FLAG'))).select(
     [col("cust_flg." + cols) for cols in cust_flag.columns]+[col("FIRST_PRD_SALES_FLAG_1")])

# COMMAND ----------

# adding FIRST_CUST_PRD_SALES_FLAG_1
ls_final_25 =prd_sales_flg.alias("cust_prd_flg").join(first_cust_matr.alias("fst"),on = [prd_sales_flg["kunag"] == first_cust_matr["cust_id"],prd_sales_flg["product_line"] == first_cust_matr["product_line"]],how='left').withColumn("FIRST_CUST_PRD_SALES_FLAG_1",when(((col("cust_prd_flg.kunag") == col("fst.cust_id")) & (col("cust_prd_flg.product_line") == 
col("fst.product_line"))),0).otherwise(col('cust_prd_flg.FIRST_CUST_PRD_SALES_FLAG'))).select(
     [col("cust_prd_flg." + cols) for cols in prd_sales_flg.columns]+[col("FIRST_CUST_PRD_SALES_FLAG_1")])

# COMMAND ----------

# calculating total freight
ls_final_26 = ls_final_25.withColumn("total_freight", when((col("vkorg") == '1000'), (col("zf00") - col("zsc_frgt"))).when(((col("vkorg") == '2000') | (col("vkorg") == '3000')), col("frgt_zf01") + col("frgt_zf02")).otherwise(0))

# COMMAND ----------

# defining billing status
ls_final_27 = ls_final_26.withColumn("last_executed_time", lit(last_processed_time_str)).withColumn("status",when(((col("fksto").isNull()) | (col("fksto") =='' )),"BILLED").otherwise("CANCELLED"))
...
# getting req columns for geo hierarchy
ls_final_28 = ls_final_27.alias("fact").join(final_geo_df.alias("Geo_data"), on = [
                                                                  ls_final_27["vkorg"] == final_geo_df["SBU"],
                                                                                    ls_final_26["product_line"] == final_geo_df["product_line"],
  ls_final_27["SALES_ORG_TYPE"] == final_geo_df["SALE_TYPE"],
  ls_final_27["PSTLZ"] == final_geo_df["PINCODE"]
], how = 'left').select(
  [col("fact." + cols) for cols in ls_final_27.columns] + [col("DISTRICT"),col("SALES_GROUP"),col("STATE"),col("ZONE"),col("COUNTRY")])
...
ls_final_29 = ls_final_28.alias("fact_ship").join(final_geo_df_ship.alias("Geo_data_ship"), on = [
                                                                  ls_final_28["vkorg"] == final_geo_df_ship["SBU"],
                                                                                    ls_final_28["product_line"] == final_geo_df_ship["product_line"],
  ls_final_28["SALES_ORG_TYPE"] == final_geo_df_ship["SALE_TYPE"],
  ls_final_28["shipto_pstlz"] == final_geo_df_ship["PINCODE_SHIP"]
], how = 'left').select(
  [col("fact_ship." + cols) for cols in ls_final_28.columns] + [col("DISTRICT_SHIP"),col("SALES_GROUP_SHIP"),col("STATE_SHIP"),col("ZONE_SHIP"),col("COUNTRY_SHIP")])

# COMMAND ----------

# selecting columns in required order
ls_final = ls_final_29.selectExpr("vbeln", "vgbel", "vgpos" , "aubel", "aupos", "fkimg","QUANTITY_EA","lgort", "matnr", "matkl", "posnr", "voleh",
                                "spart", "brgew", "vrkme", "werks", "cmpre", "maktx", "netwr", "mwsbp",
                                "total", "prsdt", "gewei", "fkdat", "kunag",
                                 "vkorg", "vtweg", "inco1", "inco2", "konda", "waerk", "fkart", "zzveh_no","kondm", "knumv",
                                "zzlr_no", "zzdr_no", "zzewaybill_no", "regio", "volum","umren","umrez","TRANS_TYPE","belnr","bukrs","gjahr","bupla","exdat","exnum","bzirk","kdgrp","vkgrp","vkbur","kunrg","payer_adrnr","sold_kunnr","agent_kunnr","sold_adrnr","agent_adrnr","agent_part_kunnr","agent_part_adrnr","bill_kunnr","bill_adrnr","ship_kunnr","ship_adrnr","sales_pernr","route","mfrgr","lifnr","bstdk","bstnk","erdat","segment","application","order_type","so_vbeln","posnr_va","cust_type","shipto_pstlz","pstlz","zasval","zsc_frgt","freight","trade_dis","jkfc_dis","jkfc_rate","p_jexp","p_zetx","p_jcst","p_zvat","p_zgta","p_ztcs","p_zeto","p_zssc","p_zb01","zrdf","zifr","zpak","zfob","zcmp","cgst_rate","cgst_amnt","sgst_rate","sgst_amnt","igst_rate","igst_amnt","ugst_rate","ugst_amnt","zf00","frgt_zf02","frgt_zf01","zbpb_pu","ztaxturn","zbasfin","zmul","zins","zmrp","zd01","zd02","zd03","zd04","zd05","zod1","zod2","zod4","zod5","zod6","zrpb","zrpc","zbasfin1","product_line","channel","sales_org_type","sales_type","total_sales_value","FIRST_CUST_FLAG_1 as FIRST_CUST_FLAG","FIRST_PRD_SALES_FLAG_1 as FIRST_PRD_SALES_FLAG","FIRST_CUST_PRD_SALES_FLAG_1 as FIRST_CUST_PRD_SALES_FLAG","total_freight","last_executed_time","status","DISTRICT","SALES_GROUP","STATE","ZONE","COUNTRY","DISTRICT_SHIP","SALES_GROUP_SHIP","STATE_SHIP","ZONE_SHIP","COUNTRY_SHIP")
...
fact_sales_invoice_incr, unmapped_final_query = surrogate_mapping_hil(csv_data, table_name, "ZSUM2", processed_db_name)

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC DROP TABLE IF EXISTS temp_zsum2;
# MAGIC DROP TABLE IF EXISTS temp_zsum2_1;

# COMMAND ----------

fact_sales_invoice_incr.write.saveAsTable("temp_zsum2")

# COMMAND ----------

# merging the incremental data to existing data on invoice id and line id
spark.sql("MERGE INTO pro_prod.fact_sales_invoice as A USING temp_zsum2 as B ON A.INVOICE_ID = B.INVOICE_ID AND A.INVOICE_LINE_ID = B.INVOICE_LINE_ID WHEN MATCHED THEN UPDATE SET * WHEN NOT MATCHED THEN INSERT * ")
